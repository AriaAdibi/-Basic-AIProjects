...
def steepest_ascent_hill_climbing( start, goal, heuristic, time_limit ):
    is_out_of_time= False
    timeout= time.time() + TIME_LIMIT

    current= start
    optimal_path= [ current ]
    current_h_cost= heuristic(current, goal)

    while is_goal(current, goal) is not True: #TODO do better implementaion
        ...
        best_h_cost= np.inf
        for next, move_cost in get_neighbors_with_move_cost(current, MOVEMENTS):
            if best_h_cost > move_cost + heuristic(next, goal) :
                best_h_cost= move_cost + heuristic(next, goal)
                current= next

        if( best_h_cost > current_h_cost ): #dead lock
            break
        else:
            current_h_cost= best_h_cost
            optimal_path.append(current)

    #############################
    if( is_goal(current, goal) is not True ): #no guaranteed optimal path to goal is found
        return is_out_of_time, None

    return is_out_of_time, optimal_path #optimal path here is reversed --> does not matter #TODO
...
def show_the_thresholded_map_with_the_optimal_path(optimal_path, optimal_path2):
    ...
    if optimal_path is not None:
        ...
        main_ax.plot(x_optimal_path, y_optimal_path, color= 'red', linewidth= 3, alpha= 0.6, label= 'A*: An Optimal Path')

    if optimal_path2 is not None: #TODO
        x_optimal_path2= []; y_optimal_path2= []
        for p in optimal_path2:
            x, y= indx_to_coord(p)
            x_optimal_path2.append(x)
            y_optimal_path2.append(y) #use zip* instead??! #TODO

        main_ax.plot(x_optimal_path2, y_optimal_path2, color= 'green', linewidth= 3, alpha= 0.6, label= 'Hill Climing: An Optimal Path(?)')
...
if (__name__ == '__main__'):
    ...
    sav_optimal_path= optimal_path #TODO
    if is_out_of_time == True:
        print('Hello from A*: Time is up. A (guaranteed) optimal path is not found.')
    elif optimal_path == None:
        print('Hello from A*: Due to blocks, no path is found. Please change the map and try again.')
    else:
        print('Hello from A*: The optimal path is found and will be shown shortly. DONE.')


    is_out_of_time, optimal_path= steepest_ascent_hill_climbing(start, goal, naive_heuristic, TIME_LIMIT)
    if is_out_of_time == True:
        print('Hello from Hill Climbing: Time is up. A (guaranteed) optimal path is not found.')
    elif optimal_path == None:
        print('Hello from Hill Climbing: Due to blocks, no path is found. Please change the map and try again.')
    else:
        print('Hello from Hill Climbing: The optimal path is found and will be shown shortly. DONE.')

    show_the_thresholded_map_with_the_optimal_path(sav_optimal_path, optimal_path)
