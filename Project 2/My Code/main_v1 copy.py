...
def steepest_ascent_hill_climbing( start, goal, heuristic, time_limit ):
    ...
    while is_goal(current, goal) is not True: #TODO do better implementaion
        ...
        best_h_cost= np.inf
        for next, move_cost in get_neighbors_with_move_cost(current, MOVEMENTS):
            if( ( best_h_cost > move_cost + heuristic(next, goal) ) and best_h_cost > current_h_cost ):
                best_h_cost= move_cost + heuristic(next, goal)
                current= next

        if( best_h_cost == np.inf ): #dead lock
            break
        else:
...
if (__name__ == '__main__'):
    ...
    if is_out_of_time == True:
        print('Hello from A*: Time is up. A (guaranteed) optimal path is not found.') #TODO Hello from
    ...
    is_out_of_time, optimal_path= steepest_ascent_hill_climbing(start, goal, moving_towards_heuristic, TIME_LIMIT)
    if is_out_of_time == True:
        print('Hello from A*: Time is up. A (guaranteed) optimal path is not found.') #TODO Hello from
    elif optimal_path == None:
        print('Hello from A*: Due to blocks, no path is found. Please change the map and try again.')
...
# in calc_vocab_ptype_freq_log10prob
    # ptype= 'story'
    # word= 'data'
    # print(X)
    # print()
